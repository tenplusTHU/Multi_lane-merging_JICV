"""
    ...
    Update-time: 2023-02-15  # V2.1 在原有矩形的基础上继承，将动态冲突，将其重要性进行提升，作为 Cyber Plane 中最重要的步骤; 与
    search 函数进行了融合
"""
        ...
        for LC_numP in self.LC_M1_cases:
            for LC_vehsName_M1 in self.LC_M1_cases[LC_numP]:
                ...
                print(f'**** scheduling, case: {case_id}/{self.LC_cases_num}, LC_vehsName_M1: {LC_vehsName_M1} \n'
                      f'     J: %.1f s, time counting: %.3f s' % (schedule.J_total, schedule.t_search_all))
                print(f'     J_M1: %.1f s,    J_M2:  %.1f s' % (schedule.s_01.optimal_J1[0], schedule.s_12.optimal_J2[0]))
...
def defaultNetwork_demo():
    ...
    r = Road()
    r.x_lim = [-600, 50]
    ax_road = r.plot(FLAG_fillRoad=True)

    for veh in vehs_All.values():
        veh.plot(r.ax_road, FLAG_speedColor=False)

    ax_road.xaxis.set_tick_params(labelsize=11)
    ax_road.set_xlabel(r'$p~\mathrm{(m)}$', {'size': 12})
    plt.tight_layout()
    plt.savefig('scenario_case.svg')
    ...
    network = VehsNetwork(vehs_R0, vehs_M1, vehs_M2,
                          rootVehL_M1=None, rootVehL_M2=None,
                          LC_max_numP=3, LC_max_sizeP_CAVs=2)
...
if __name__ == '__main__':
    ...
    # ax_whole.set_xlim([-0.03, 1.02])
    # ax_whole.set_ylim([-1.35, 1.35])
    ...
    # LC_vehsPos_M1 = [1, 4]
    # ax_lo, ax_up = vn.showDCG_subs(LC_vehsPos_M1=LC_vehsPos_M1)
    # ax_lo.set_xlim([-0.03, 1.02])
    # ax_up.set_xlim([-0.03, 1.02])
    # plt.savefig('split.svg')
    ...
    # plt.savefig('optSche_net.svg')

    ''' print each node's cost '''
    for node in vn.optSchedule.optSeq_Lane1_nodes:
        print(node.veh_name, f'%.1f' % node.j)

    print('\n')
    for node in vn.optSchedule.optSeq_Lane2_nodes:
        print(node.veh_name, f'%.1f' % node.j)

    show_trajs_threeLanes(vn.vehs_All)
    plt.savefig('optSche_trajs.svg')
