...
class Long_StatePhase:
    ...
    def __get_opt_tf(self):
        if self.a0 > 0 and self.bbc.stage1_FLAG is False and self.bbc.stage0_FLAG is False:  # debug, 2023-02-14
        ...
        for pf in adjustD_pf:
            if not self.bbc.stage1_FLAG and not self.bbc.stage0_FLAG:  # in most cases
    ...
    def __get_opt_pf(self):
        ...
        vMin_list = np.linspace(self.v0, self.v_min, num_points, endpoint=True)
        boundP_tf, boundP_pf = [], []
    ...
    def plot_phase(self, ax=None, c=None, input_tf=None, input_pf=None, overlap_FLAG='sweet'):
        ...
        ''' temp '''
        # ax_road.set_xlim([8, 31])
        # ax_road.set_ylim([60, Road.MERGE_ZONE_LENGTH])
        ...
        # ax.plot(self.adjustD_tf, self.adjustD_pf, c='b' if not c else c, lw=lw, zorder=7,
        #         label=r'$j_\mathrm{max}~\to~0~\to~j_\mathrm{min}$')  # label='optimal-time'
        ax.plot(self.boundP_tf[:-2], self.boundP_pf[:-2], c='b' if not c else c, lw=lw, zorder=7,
                label=r'$j_\mathrm{min}~\to~j_\mathrm{max}~\to~j_\mathrm{min}$')  # label='shortest-distance'
        ax.plot(self.boundP_tf[-3:], self.boundP_pf[-3:], c='k' if not c else c, lw=lw, zorder=7,
                label=r'$j_\mathrm{min}~\to~j_\mathrm{max}~\to~0$' + '\n' + r'$\to~j_\mathrm{max}~\to~j_\mathrm{min}$')  # label='shortest-distance'
        ...
        ax.fill_betweenx(self.adjustD_pf, self.adjustD_tf, (self.adjustD_pf - self.pole[1]) / self.v_des + self.pole[0],
                         fc='g' if not c else c, alpha=alpha, zorder=3)  # label='sweet zone'
        if self.boundP_cutOff_FLAG:
            ax.fill_between(self.boundP_tf, self.boundP_pf, np.interp(self.boundP_tf, self.adjustD_tf, self.adjustD_pf),
                            fc='g' if not c else c, alpha=alpha, zorder=5)  # label='normal zone'
        else:
            ax.fill_between(self.boundP_tf[:-2], self.boundP_pf[:-2],
                            np.interp(self.boundP_tf[:-2], self.adjustD_tf, self.adjustD_pf), fc='g' if not c else c,
...
if __name__ == '__main__':
    ...
    t1, p1, v1, a1 = 0, -60, 10, 0.2
    v_des, v_min = 20, 5
    route = 'R0'

    s1 = Long_StatePhase(t1, p1, v1, a1, route=route, v_des=v_des, v_min=v_min)
    ax = s1.plot_phase(input_tf=17.5, input_pf=160)

    ''' modify for paper IV '''
    plt.tick_params(labelsize=11)
    font_label = {'size': 12}
    ax.set_xlabel(r'$t_{f}$ (s)', font_label)
    ax.set_ylabel(r'$p_{k,f}$ (m)', font_label)

    ax.set_xlim([7, 35])
    ax.set_ylim([60, 200])

    # ax.set_xticks([s1.pole[0], s1.boundP_tf[-1]])
    # ax.set_xticklabels([r'$t_{j, \mathrm{pole}}$', r'$t_{j, \mathrm{end}}$'], fontsize=12)

    # ax.set_yticks([s1.pole[1], Road.MERGE_ZONE_LENGTH])
    # ax.set_yticklabels([r'$p_{j,\mathrm{pole}}$', r'$p_{\mathrm{end}}$'], fontsize=12, rotation=90)

    plt.legend(loc=4, fontsize=10)
    plt.savefig('statePhase.svg', dpi=600)
    ...
    """ **** Debug **** """
    # t0, p0, v0, a0 = 33.3, -444.4389, 17.477, -1.74
    # v_des, v_min = 20, 10
